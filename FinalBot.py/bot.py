...
from les10 import pars
from newbot import currency
from homework import spb
token = '6269470010:AAFFI4HFv7_l4ki_W2pe4PULjndqhpKfHG4'
...
def answer(message):
    if message.text.lower() == 'отлично':
        ...
        bot.register_next_step_handler(message, valuta)
    elif message.text.lower() == 'погода':
        bot.send_message(message.chat.id, 'Введите день: ДД.ММ.ГГГГ')
...
@bot.message_handler(content_types=['text'])
def pogoda(message):
    bot.send_message(message.chat.id, spb(message.text))
